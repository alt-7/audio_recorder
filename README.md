# 🎙 Audio Recording Service (Yii2 + Docker)

Сервис для записи и обработки аудио. Реализован на PHP 8.2 (Yii2 Basic), PostgreSQL и FFmpeg.

## ✨ Возможности
* **Запись аудио:** Загрузка Base64 аудиопотока (WebM/WAV/etc).
* **Обработка (FFmpeg):**
   * Удаление тишины в начале и конце.
   * Умное шумоподавление (`highpass`, `afftdn`).
   * Нормализация громкости (`loudnorm`).
   * Конвертация в MP3 (VBR High Quality).
* **Безопасность:**
   * Rate Limiting (защита от DDOS).
   * Валидация MIME-типов, имен файлов, входных данных.
   * API Key Аутентификация.
   * Защита от SQL-инъекций
   * Санитизация имен файлов и директорий
* **Документация:** Swagger / OpenAPI.
* **Тесты:** Unit и API (Codeception).

---

## 🚀 Установка и запуск

### Требования
* Docker & Docker Compose

# Клонирование репозитория:
git clone https://github.com/alt-7/audio_recorder.git
cd project

# Создаем файл настроек из шаблона
cp .env.example .env

# Собрать и запустить контейнеры
docker compose up -d --build

*При запуске скрипт `entrypoint.sh` автоматически:*
* *Установит зависимости Composer*
* *Выполнит миграции базы данных*
* *Сгенерирует документацию Swagger*

# Контейнер
docker compose exec app sh

# Запуск всех тестов
vendor/bin/codecept run

# Только API тесты
vendor/bin/codecept run api

# Только Unit тесты
vendor/bin/codecept run unit

# Создание пользователей (после запуска теста база очистится)
php yii user/init

# Генерация Swagger JSON
php yii swagger/generate

# Очистить кеш
php yii cache:flush-all


# 📚 Документация API
После запуска сервис доступен по адресу: 👉 http://localhost:8000/swagger/doc
# Доступы:
API Key: secret-api-key-123 (передавать в заголовке X-Api-Key)
